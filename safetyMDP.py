...
class SafetyMDP(mdpUtil.MDP):
    ...
    def succAndProbReward(self, state, action):
        ...
        if row == self.endRow and col == self.endRow:
            #rint state
            return []
        ...
        if action == "U":
            newRow = row - 1
            newCol = col
            if (newRow, newCol) in path:
                # Game over go to end
                newState = (self.endRow, self.endCol, path)
                return [(newState, prob, -1000000)]
            else:
                path = path + ((newRow, newCol),)
                newState = (newRow, newCol, path)
                if self.isEnd(newRow, newCol):
                    reward = 0
                    numEdges = len(path)
                    for loc in path:
                        reward += self.locationGrid[loc[0]][loc[1]][0]
                    reward = -100 * reward / (float) (numEdges)
